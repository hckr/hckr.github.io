if (
    ...
    !document.cookie
        .split(';')
        .map(c => c.trim())
        .includes('cookieAlertDismissed=1')
) {
    let message, closeText;
    switch (document.documentElement.lang) {
        case 'pl':
            message = `
            Ta strona wykorzystuje pliki cookie do celów statystycznych (Google Analytics, <a style="text-decoration: underline; color: inherit" href="https://www.google.com/policies/privacy/partners/">polityka prywatności</a>).
            W ustawieniach przeglądarki można zablokować odpowiednie pliki cookie.`;
            closeText = 'zamknij';
            break;
        default:
            message = `
            This website uses cookies for statistical purposes (Google Analytics, <a style="text-decoration: underline; color: inherit" href="https://www.google.com/policies/privacy/partners/">privacy policy</a>).
